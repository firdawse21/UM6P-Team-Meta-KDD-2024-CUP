...
class RAG_KG_Model:
    ...
    def generate_hypothetical_answer(self,query):
      prompt = "answer the question only if you are sure about the answer if not return the same query , query : "+query
      ...
      print(f"Response: {response}")  # Debugging line to print the response
      return response[0].outputs[0].text  # Adjust this line based on the actual structure
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
              ...
              answer_embedding = self.calculate_embeddings([hypothetical_answer])[0].reshape(1, -1)
              ...
              relevant_faiss_index = faiss.IndexFlatIP(relevant_chunks_embeddings.shape[1])
              relevant_faiss_index.add(relevant_chunks_embeddings)
              _, indices = relevant_faiss_index.search(answer_embedding ,30)
              retrieval_results = relevant_chunks[indices[0]]
              pairs = []
              for doc in retrieval_results:
                  pairs.append([query, doc])
              scores = self.cross_encoder.predict(pairs)
              scored_docs = zip(scores,retrieval_results)
              sorted_docs =  sorted(scored_docs, key=lambda x: x[0], reverse=True)
              reranked_docs = [doc for _, doc in sorted_docs]
              reranked_docs.insert(0, hypothetical_answer)
    ...
    def format_prompts(self, queries, query_times, batch_retrieval_results=[], batch_kg_results=[]):
        ...
        """        
        system_prompt = "You are provided with a question and various references. Your task is to answer the question succinctly, using the fewest words possible.If the question is based on a false premise or assumption, respond with 'invalid question.'.If the references do not contain the necessary information to answer the question, respond with 'I don't know'. There is no need to explain the reasoning behind your answers."
        formatted_prompts = []
