...
def load_data_in_batches(dataset_path, batch_size):
    ...
    Each batch is a dictionary containing lists of interaction_ids, queries, search results, query times, answers, and splits.
        ...
        with bz2.open(dataset_path, "rt") as file:
            ...
            for line in file:
                try:
                    ...
                    for key in batch:
                        batch[key].append(item.get(key, None))
...
def generate_predictions(dataset_path, participant_model):
    ...
    """
    queries, ground_truths, predictions , retrieved_docs, queryTypes= [], [], [],[],[]
    ...
    for batch in tqdm(load_data_in_batches(dataset_path, batch_size), desc="Generating predictions"):
        ...
        batch_splits = batch.get("split", [])  # Get the split values for the batch
        batch_queryType = batch.get("question_type", [])

        for interaction_id, query, ground_truth, split, batch_queryType in zip(batch["interaction_id"], batch["query"], batch_ground_truths, batch_splits, batch["question_type"]):
            if prediction_count >= max_predictions:
                break
            ...
            prediction_batch = {
                ...
                "query": [query],
                "search_results": batch["search_results"],  # Include search_results if required
                "query_time": batch["query_time"]  # Include query_time if required
            }
            #deleted[0]
            prediction = participant_model.batch_generate_answer(prediction_batch)  # Generate prediction for the current query
            print(prediction)
            queries.append(query)
            ground_truths.append(ground_truth)
            predictions.append(prediction)
            #retrieved_docs.append(reranked_docs)
            queryTypes.append(batch_queryType)
            prediction_count += 1
    
    return queries, ground_truths, predictions, queryTypes
...
if __name__ == "__main__":
    ...
    queries, ground_truths, predictions, queryTypes = generate_predictions(DATASET_PATH, participant_model)
    print(queries)
    print(predictions)
    ...
    with open('faiss_sentence_reranker_hyde_mmr.json', 'w') as json_file:
